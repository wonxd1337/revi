# main.py - Dioptimalkan
import os
import sys
import signal
import atexit
import time
from config import Config
from proxy_manager import ProxyManager
from cache_manager import CacheManager
from scanner import MovableTypeScanner
from ip_generator import IPGenerator

class MovableTypeMassScanner:
    def __init__(self):
        self.proxy_manager = None
        self.cache_manager = None
        self.scanner = None
        self.running = True
        
        # Setup signal handlers
        signal.signal(signal.SIGINT, self.signal_handler)
        signal.signal(signal.SIGTERM, self.signal_handler)
        
        # Register cleanup
        atexit.register(self.cleanup)
    
    def signal_handler(self, signum, frame):
        """Handle Ctrl+C"""
        print("\n\n[!] Received interrupt signal. Cleaning up...")
        self.running = False
        sys.exit(0)
    
    def cleanup(self):
        """Bersihkan resources"""
        print("[*] Cleaning up resources...")
        
        if self.proxy_manager:
            self.proxy_manager.cleanup()
            self.proxy_manager.print_stats()
        
        if self.cache_manager:
            stats = self.cache_manager.get_stats()
            print(f"\n[*] Cache stats: {stats}")
        
        print("[✓] Cleanup completed")
    
    def print_header(self):
        """Tampilkan header"""
        print("""
    ╔════════════════════════════════════════════════════════════╗
    ║         Movable Type Mass Scanner v3.0 (Optimized)        ║
    ║  - Sequential IP Processing (Reverse → Scan → Next)       ║
    ║  - On-Demand Working Proxy Validation                     ║
    ║  - Automatic Proxy Retry Until Success                    ║
    ║  - SQLite Cache System                                     ║
    ╚════════════════════════════════════════════════════════════╝
        """)
    
    def run(self):
        """Main execution"""
        self.print_header()
        
        # Inisialisasi komponen
        print("[*] Initializing components...")
        
        # Proxy Manager
        print("[*] Starting Proxy Manager...")
        self.proxy_manager = ProxyManager()
        
        # Cache Manager
        print("[*] Starting Cache Manager...")
        self.cache_manager = CacheManager()
        
        # Scanner
        self.scanner = MovableTypeScanner(self.proxy_manager, self.cache_manager)
        
        # IP Generator
        self.ip_generator = IPGenerator(self.cache_manager)
        
        # Pilih metode
        print("\n" + "="*50)
        print("SELECT INPUT METHOD")
        print("="*50)
        print("1. Scan from IP list file")
        print("2. Scan with RNG IP (auto-generate)")
        print("3. Continuous scan (run forever)")
        print("4. Show statistics")
        
        choice = input("\nChoice (1-4): ").strip()
        
        if choice == '1':
            self.scan_from_file()
        elif choice == '2':
            self.scan_with_rng()
        elif choice == '3':
            self.continuous_scan()
        elif choice == '4':
            self.show_stats()
        else:
            print("[!] Invalid choice!")
    
    def scan_from_file(self):
        """Scan dari file (sequential)"""
        filename = input("IP list file: ").strip()
        
        try:
            with open(filename, 'r') as f:
                ips = [line.strip() for line in f if line.strip()]
            
            print(f"[*] Loaded {len(ips)} IPs")
            print("[*] Processing IPs sequentially...\n")
            
            processed = 0
            for ip in ips:
                if not self.running:
                    break
                
                # Proses IP satu per satu
                self.scanner.process_ip(ip)
                processed += 1
                
                # Tampilkan progress
                print(f"\n[Progress] {processed}/{len(ips)} IPs processed")
            
            print(f"\n[✓] All IPs processed: {processed}/{len(ips)}")
                        
        except Exception as e:
            print(f"[!] Error: {e}")
    
    def scan_with_rng(self):
        """Scan dengan RNG IP (sequential)"""
        base_ip = input("Base IP (e.g., 157.7.44): ").strip()
        
        # Format IP
        if base_ip.count('.') == 2:
            base_ip = base_ip + '.1'
        
        # Jumlah IP
        try:
            max_valid = input(f"Number of IPs (default {Config.MAX_VALID_RNG}): ").strip()
            if max_valid:
                max_valid = min(int(max_valid), Config.MAX_VALID_RNG_LIMIT)
            else:
                max_valid = Config.MAX_VALID_RNG
        except:
            max_valid = Config.MAX_VALID_RNG
        
        print(f"[*] Will generate up to {max_valid} valid IPs")
        print("[*] Processing IPs sequentially...\n")
        
        # Stream dan proses IP satu per satu
        processed = 0
        for ip in self.ip_generator.generate_ips(base_ip, max_valid):
            if not self.running:
                break
            
            self.scanner.process_ip(ip)
            processed += 1
            
            print(f"\n[Progress] {processed}/{max_valid} IPs processed")
        
        print(f"\n[✓] Completed: {processed} IPs processed")
    
    def continuous_scan(self):
        """Continuous scan mode (sequential)"""
        print("\n[*] Continuous Scan Mode")
        print("[*] Press Ctrl+C to stop\n")
        
        base_ip = input("Base IP for generation: ").strip()
        
        if base_ip.count('.') == 2:
            base_ip = base_ip + '.1'
        
        cycle = 0
        while self.running:
            cycle += 1
            print(f"\n{'='*70}")
            print(f"CYCLE #{cycle} - {time.strftime('%Y-%m-%d %H:%M:%S')}")
            print('='*70)
            
            # Generate dan proses IP satu per satu
            processed = 0
            for ip in self.ip_generator.generate_ips(base_ip, Config.MAX_VALID_RNG):
                if not self.running:
                    break
                
                self.scanner.process_ip(ip)
                processed += 1
            
            print(f"\n[Cycle #{cycle}] Processed {processed} IPs")
            
            # Tampilkan statistik
            self.show_stats(quiet=True)
            
            # Istirahat antar cycle
            if self.running:
                print(f"\n[*] Cycle #{cycle} completed. Waiting 60 seconds...")
                for i in range(60):
                    if not self.running:
                        break
                    time.sleep(1)
                    if i % 10 == 0 and i > 0:
                        print(f"[*] Next cycle in {60-i}s...", end='\r')
    
    def show_stats(self, quiet=False):
        """Tampilkan statistik"""
        if not quiet:
            print("\n" + "="*60)
            print("SYSTEM STATISTICS")
            print("="*60)
        
        # Cache stats
        cache_stats = self.cache_manager.get_stats()
        print(f"\n📦 Cache Database:")
        for table, count in cache_stats.items():
            if table != 'db_size_mb':
                print(f"  - {table}: {count} entries")
        if 'db_size_mb' in cache_stats:
            print(f"  - DB Size: {cache_stats['db_size_mb']:.2f} MB")
        
        # Proxy stats
        self.proxy_manager.print_stats()
        
        # File stats
        print("\n📁 Output Files:")
        for name, filename in Config.OUTPUT_FILES.items():
            if os.path.exists(filename):
                size = os.path.getsize(filename) / 1024
                print(f"  - {filename}: {size:.2f} KB")
            elif os.path.exists(Config.TEMP_DIR + filename):
                size = os.path.getsize(Config.TEMP_DIR + filename) / 1024
                print(f"  - {filename} (temp): {size:.2f} KB")

def main():
    scanner = MovableTypeMassScanner()
    scanner.run()

if __name__ == "__main__":
    main()